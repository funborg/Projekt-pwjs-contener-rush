...
export default class InstrScene extends Phaser.Scene {
    ...
    create() {
        ...
        var lineOne = new Phaser.Geom.Line(innerWidth / 2 - 480, innerHeight / 2 - 260, innerWidth / 2 + 420, innerHeight / 2 - 260);
        var graphics = this.add.graphics({ lineStyle: { width: 10, color: 0x012636 } });
        graphics.strokeLineShape(lineOne);

        //adding 'MOVEMENT' text
        this.movement = this.add.text(innerWidth / 2 - 260, innerHeight / 2 - 200, 'MOVEMENT', { fontFamily: 'Stencil', fontSize: 46, color: '#085b80' }).setOrigin(0.5);
        ...
        //w
        this.wButton = this.add.text(innerWidth / 2 - 260, innerHeight / 2 - 120, 'MOVE FORWARD : W', { fontFamily: 'Stencil', fontSize: 24, color: '#085b80' }).setOrigin(0.5);
        this.wButton.setStroke('#133b54', 8);
        this.wButton.setShadow(1, 2, '#000000');

        //s
        this.sButton = this.add.text(innerWidth / 2 - 260, innerHeight / 2 - 40, 'MOVE BACKWARD : S', { fontFamily: 'Stencil', fontSize: 24, color: '#085b80' }).setOrigin(0.5);
        this.sButton.setStroke('#133b54', 8);
        this.sButton.setShadow(1, 2, '#000000');

        //a
        this.aButton = this.add.text(innerWidth / 2 - 260, innerHeight / 2 + 40, 'TURN LEFT : A', { fontFamily: 'Stencil', fontSize: 24, color: '#085b80' }).setOrigin(0.5);
        this.aButton.setStroke('#133b54', 8);
        this.aButton.setShadow(1, 2, '#000000');

        //d
        this.dButton = this.add.text(innerWidth / 2 - 260, innerHeight / 2 + 120, 'TURN RIGHT : D', { fontFamily: 'Stencil', fontSize: 24, color: '#085b80' }).setOrigin(0.5);
        ...
        //adding 'OTHER' text
        this.other = this.add.text(innerWidth / 2 + 240, innerHeight / 2 - 200, 'OTHER', { fontFamily: 'Stencil', fontSize: 46, color: '#085b80' }).setOrigin(0.5);
        this.other.setStroke('#133b54', 10);
        this.other.setShadow(1, 2, '#000000');

        //space
        this.spaceButton = this.add.text(innerWidth / 2 + 240, innerHeight / 2 - 120, 'BREAK : SPACE', { fontFamily: 'Stencil', fontSize: 24, color: '#085b80' }).setOrigin(0.5);
        this.spaceButton.setStroke('#133b54', 8);
        this.spaceButton.setShadow(1, 2, '#000000');

        //e
        this.eButton = this.add.text(innerWidth / 2 + 240, innerHeight / 2 - 40, 'INTERACT : E', { fontFamily: 'Stencil', fontSize: 24, color: '#085b80' }).setOrigin(0.5);
        ...
        var lineTwo = new Phaser.Geom.Line(innerWidth / 2 - 490, innerHeight / 2 + 170, innerWidth / 2 + 420, innerHeight / 2 + 170);
        var graphics = this.add.graphics({ lineStyle: { width: 10, color: 0x012636 } });
        ...
        this.instruction1 = this.add.text(innerWidth / 2, innerHeight / 2 + 200, 'Take the ship to the port', { fontFamily: 'Stencil', fontSize: 28, color: '#085b80' }).setOrigin(0.5);
        this.instruction1.setStroke('#133b54', 8);
        this.instruction1.setShadow(1, 2, '#000000');

        this.instruction2 = this.add.text(innerWidth / 2, innerHeight / 2 + 250, 'Load the container', { fontFamily: 'Stencil', fontSize: 28, color: '#085b80' }).setOrigin(0.5);
        this.instruction2.setStroke('#133b54', 8);
        this.instruction2.setShadow(1, 2, '#000000');

        this.instruction3 = this.add.text(innerWidth / 2, innerHeight / 2 + 300, 'Transport it to the indicated place', { fontFamily: 'Stencil', fontSize: 28, color: '#085b80' }).setOrigin(0.5);
        this.instruction3.setStroke('#133b54', 8);
        this.instruction3.setShadow(1, 2, '#000000');

        this.instruction4 = this.add.text(innerWidth / 2, innerHeight / 2 + 350, 'Earn points and gain more game time!', { fontFamily: 'Stencil', fontSize: 28, color: '#085b80' }).setOrigin(0.5);
        this.instruction4.setStroke('#133b54', 8);
        this.instruction4.setShadow(1, 2, '#000000');

        //adding 3rd decorational line
        var lineThree = new Phaser.Geom.Line(innerWidth / 2 - 490, innerHeight / 2 + 390, innerWidth / 2 + 420, innerHeight / 2 + 390);
        var graphics = this.add.graphics({ lineStyle: { width: 10, color: 0x012636 } });
        graphics.strokeLineShape(lineThree);

        //creating menu & restart buttons 
        this.menuB = new menuButton(this, innerWidth / 2 - 250, innerHeight / 2 + 450);
        this.startB = new startButton(this, innerWidth / 2 + 180, innerHeight / 2 + 450, 'GameScene');
    }
    update() {
